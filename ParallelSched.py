...
def schedule_sessions(group_sessions, joint_sessions, strict_non_overlaps, prioritized_non_overlaps, preferences, impossible_slots, num_sessions, num_tracks):
    ...
    # Add impossible slots constraints (0-based indexing for sessions)
    ...
    # Objective: Minimize penalties for soft constraints
    ...
    if status == cp_model.FEASIBLE or status == cp_model.OPTIMAL:
        # Dictionary to store the agenda by session
        session_agenda = {sess: [] for sess in range(1, num_sessions + 1)}

        # Collect group assignments by session
        for group, sessions in group_vars.items():
            for i, session in enumerate(sessions):
                sess = solver.Value(session) + 1  # Session number (1-based)
                session_agenda[sess].append(f'{group}')

        # Collect joint session assignments by session
        for idx, joint in enumerate(joint_sessions):
            joint_session = solver.Value(joint_session_vars[idx]) + 1  # Session number (1-based)
            joint_groups = " + ".join(joint)
            session_agenda[joint_session].append(f'Joint {joint_groups}')

        # Print the agenda grouped by session
        print("Agenda by Session:")
        for sess in sorted(session_agenda.keys()):
            print(f"\nSession {sess}:")
            for item in session_agenda[sess]:
                print(f"  {item}")
    else:
